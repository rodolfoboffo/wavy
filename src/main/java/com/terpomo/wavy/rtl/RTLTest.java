...
public class RTLTest {
    ...
    public static void main(String[] args) {
        ...
        for (int deviceIndex = 0; deviceIndex < count; deviceIndex++) {
            System.out.println(RTLAPI.INSTANCE.rtlsdr_get_device_name(deviceIndex));
            Pointer[] devicePtr = new Pointer[1];
            int result = RTLAPI.INSTANCE.rtlsdr_open(devicePtr, deviceIndex);
            System.out.println(String.format("Open result: %d", result));
            if (result >= 0) {
                result = RTLAPI.INSTANCE.rtlsdr_reset_buffer(devicePtr[0]);
                System.out.println(String.format("Reset buffer result: %d", result));
                result = RTLAPI.INSTANCE.rtlsdr_set_testmode(devicePtr[0], false);
                System.out.println(String.format("Enable test mode result: %d", result));
                result = RTLAPI.INSTANCE.rtlsdr_set_ds_mode(devicePtr[0], RTLDataSamplingMode.RTLSDR_DS_IQ.getValue(), 28000000);
                ...
                result = RTLAPI.INSTANCE.rtlsdr_set_tuner_gain(devicePtr[0], gains[gains.length-1]);
                System.out.println(String.format("Set gain result: %d", result));
                int tunerGain = RTLAPI.INSTANCE.rtlsdr_get_tuner_gain(devicePtr[0]);
                System.out.println(String.format("Tuner gain: %d", tunerGain));
                result = RTLAPI.INSTANCE.rtlsdr_set_sample_rate(devicePtr[0], 2048000);
                System.out.println(String.format("Set sample rate result: %d", result));
                int sampleRate = RTLAPI.INSTANCE.rtlsdr_get_sample_rate(devicePtr[0]);
                System.out.println(String.format("Sample Rate: %d", sampleRate));
                result = RTLAPI.INSTANCE.rtlsdr_set_center_freq(devicePtr[0], 106300000);
                System.out.println(String.format("Set frequency result: %d", result));
                long centerFreq = RTLAPI.INSTANCE.rtlsdr_get_center_freq64(devicePtr[0]);
                System.out.println(String.format("Center Frequency: %d", centerFreq));
                result = RTLAPI.INSTANCE.rtlsdr_set_direct_sampling(devicePtr[0], false);
                System.out.println(String.format("Set Direct Sample result: %d", result));
                int dsMode = RTLAPI.INSTANCE.rtlsdr_get_direct_sampling(devicePtr[0]);
                ...
                for (int i = 0; i < 13; i++) {
                    ...
                    result = RTLAPI.INSTANCE.rtlsdr_read_sync(devicePtr[0], buffer, BUFFER_SIZE, readResult);
                    System.out.println(String.format("Read result: %d", result));
                    if (result < 0) break;
                }
                result = RTLAPI.INSTANCE.rtlsdr_close(devicePtr[0]);
